...
public class MySqlHelper {
    ...
    //访问的地址 120.76.28.84:3306/nbserver
    private String URL;
    //定义数据库的连接
     ...
     * @param sql 语句
     * @param params 占位符
     * @param tJavabean javabean,会执行javabean类里面的toString方法
     * @return
     * @throws Exception
     */
    ...
    public ListGageModule getListForPage(int pagesize, int pageindex, String querysql, String strwhere, String orderby, String tablename) throws Exception {
        ListGageModule pagemodule = new ListGageModule();
        try {
            if (connection == null || connection.isClosed())
                connection = DriverManager.getConnection(URL, USERNAME, PASSWORD);//定义连接

            List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
            String sql = "{call Query_Pagination(?,?,?,?,?,?,?,?,?)}";
            CallableStatement callableStatement = connection.prepareCall(sql);
            callableStatement.setString(1, querysql);
            callableStatement.setString(2, tablename);
            callableStatement.setString(3, strwhere);
            callableStatement.setString(4, orderby);
            callableStatement.setInt(5, pageindex);
            ...
            while (resultset.next()) {
                ...
                for (int i = 0; i < columnlength; i++) {
                    //String metaDateKey = metaDate.getColumnName(i + 1);//获取列名
                    String metaDateKey = metaDate.getColumnLabel(i + 1);//获取字段别名
                    Object resultsetValue = resultset.getObject(metaDateKey);
                    if (resultsetValue == null) {
                        resultsetValue = "";
                    }
            ...
            pagemodule.setList(list);
            //pagemodule.setRS(resultset);
            pagemodule.setTotalcount(callableStatement.getInt(8));
            pagemodule.setPagecount(callableStatement.getInt(9));
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
            pagemodule.setErrstr(e.toString());
        } finally {
            closeConnection();
        }
        return pagemodule;
