...
<script type="text/javascript">
    var table = $('.table-sort').dataTable({
        ...
        "columns": [
            ...
            {
                "data": "petVaccineId", "render": function (data, type, full, meta) {
                    var opt = "";
                    if(full.isOver == 0){
                        opt += "<a title='开始接种' href='javascript:;' onclick=\"confirmShower("+data+")\" class='ml-5' style='text-decoration:none'>开始接种</a>";
                    }else if(full.isOver == 1){
                        opt += "<a title='完成接种' href='javascript:;' onclick=\"okShower("+data+")\" class='ml-5' style='text-decoration:none'>完成接种</a>";
                    }
                    return opt;
                }
            }
    ...
    function confirmShower(id) {
        layer.confirm('确认开始接种？',function(index){
            $.ajax({
                type: 'POST',
                    url: '/admin/home/confirmPetVaccine',
                data:{
                    petVaccineId:id
                },
                dataType: 'json',
    ...
    function okShower(id) {
        layer.confirm('确认完成接种？',function(index){
            var load = layer.load(1);
            $.ajax({
                type: 'POST',
                url: '/admin/home/okPetVaccine',
                data:{
                    petVaccineId:id
                },
                dataType: 'json',
